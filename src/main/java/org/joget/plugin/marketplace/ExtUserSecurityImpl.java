...
public class ExtUserSecurityImpl extends UserSecurityImpl implements HiddenPlugin {
    ...
    public String getName() {
        return "JSON Web Token User Security";
    }

    @Override
    public String getDescription() {
        return "User Security for JSON Web Token";
    }
...
//        if (redirect) {
//            content = "<script> $(document).ready(function() { var url = \"" + JsonWebTokenDirectoryManager.getCallbackURL() + "\"; $(location).attr('href',url); }); </script>\n";
//        } else {
            content += "<style>\n"
                    + "#openIDLogin {\n"
                    ...
                    + "    margin: 20px 100px;\n"
                    + "    border: 1px solid " + dmImpl.getPropertyString("buttonBorderColor") + ";\n"
                    + "    font-weight: 600;\n"
                    + "}\n"
                    + "#icon {\n"
                    + "padding-right: 20px;\n"
                    + "}\n"
                    + "</style>";
            content += "<a href=\"" + dmImpl.getPropertyString("outh2ProxyURL") + "\" id=\"openIDLogin\"><i id=\"icon\" class=\"" + dmImpl.getPropertyString("buttonIcon") + "\"></i>" + dmImpl.getPropertyString("buttonText") + "</a>";
//        }
